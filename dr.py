...
class DR(object):
    ...
    def get_max(self,outputFN):
        ...
        #maximum_val = np.array([0.0]*20)
        #maximum_val = np.array([0.0]*18) #with addr isue
        maximum_val = np.array([0.0]*19)
        ...
        for e,fn in enumerate(sample):
          ...
          #  b = np.array([[0]*20]*self.max_len, dtype=float)
            #b = np.array([[0]*18]*self.max_len, dtype=float)
            b = np.array([[0]*19]*self.max_len, dtype=float)
    ...
    def generator(self,t,batch_size):
        ...
        while True:
            for fn in sample:
                # Only look at first max_len of data (and pad with empty feature vector)
                #b = np.array([[0]*20]*self.max_len, dtype=float)
                #b = np.array([[0]*18]*self.max_len, dtype=float) #without addr-31may2026
                b = np.array([[0]*19]*self.max_len, dtype=float)
                ...
                if len(x) == batch_size:
                    if self.normalizeFN is not None:
                        ...
                        safe_max = np.where(self.maximum_val == 0, 1, self.maximum_val)
                        yield (np.asarray(x) / safe_max, np.asarray(x) / safe_max)
