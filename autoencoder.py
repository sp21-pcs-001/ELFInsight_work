...
def define_model(kernel_size,strides,max_len):
    inputs = tf.keras.Input(shape=(max_len, 19))
    ...
    conv9 = tf.keras.layers.Conv1D(19*factor, kernel_size=kernel_size, strides=strides, activation='relu', padding='same', kernel_initializer='he_normal')(conv9)
    conv10 = tf.keras.layers.Conv1D(19, 1, activation='tanh')(conv9)
...
def _main():
    ...
    base = tf.keras.backend.get_value(model.optimizer.learning_rate)
    def schedule(epoch):
